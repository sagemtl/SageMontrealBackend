...
router.post("/leaderboard/:title_id", cors(corsOptions), async (req, res) => {
  try {
    ...
    const sortedLeadboard = scoresJson.sort((a, b) =>
      a.score > b.score ? -1 : 1
    );
    const rank =
      sortedLeadboard.findIndex(
        (entry) => entry.name == name && entry.score == score
      ) + 1;
    const leaderboardString = sortedLeadboard.map((entry) =>
      JSON.stringify(entry)
    );
    const queryResult2 = await db
      .collection("ktv-leaderboard")
      .findOneAndUpdate(query, { $set: { scores: leaderboardString } });
