module.exports = function (userData, originalData) {
  ...
  while (userPtr < len) {
    let currTimeDiff = Math.abs(
      originalData[originalPtr].audioTime - userData[userPtr].audioTime
    );

    while (
      originalPtr + 1 < originalData.length &&
      currTimeDiff >=
        Math.abs(
          originalData[originalPtr + 1].audioTime - userData[userPtr].audioTime
        )
    ) {
      originalPtr++;
      currTimeDiff = Math.abs(
        originalData[originalPtr].audioTime - userData[userPtr].audioTime
      );
    }

    const current_score = verifyDelta(
      userData[userPtr].audioData,
      originalData[originalPtr].audioData,
      count
    );
    score += current_score;
    userPtr++;
