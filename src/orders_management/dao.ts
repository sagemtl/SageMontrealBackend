...
class OrderManagementDAO extends DAO {
  ...
  async createOrderItems(
    ...
    const orderItemsValues = orderItems.map(
      (orderItem) =>
        `('${orderItem.sku_id}', '${orderId}', ${orderItem.quantity}, ${
          orderItem.adjusted_price ?? null
        })`
    );
    const query = `INSERT INTO ${
      this.orderItemsTable
    } (sku_id, order_id, quantity, adjusted_price) values ${orderItemsValues.join(
      ', '
    )};`;
